...
void OnAppStart()
{
	...
	wrenInterpret(vm, "main", code.c_str());

	wrenEnsureSlots(vm, 1);
	wrenGetVariable(vm, "main", "ImGuiTest", 0);
	w_ImGuiTest = wrenGetSlotHandle(vm, 0);
	w_update = wrenMakeCallHandle(vm, "update()");
}
...
void OnAppUpdate()
{
	...
	{
		wrenEnsureSlots(vm, 1);
		wrenSetSlotHandle(vm, 0, w_ImGuiTest);
		wrenCall(vm, w_update);
	}
}

void OnAppComplete()
{
	wrenReleaseHandle(vm, w_ImGuiTest);
